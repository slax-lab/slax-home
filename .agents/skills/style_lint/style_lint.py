...
# slax-home content lives under src/content/{blog,{reader,note}-{changelog,alternatives}}/.
# Scanning the whole src/content/ folder catches all collections.
DOCS_ROOT = Path("src/content")
CHANGELOG_DIRS = (DOCS_ROOT / "reader-changelog", DOCS_ROOT / "note-changelog")
# Backwards-compat alias (kept for code paths that still reference CHANGELOG_DIR).
CHANGELOG_DIR = CHANGELOG_DIRS[0]
EXCLUDED_DIRS = {"_book", "node_modules", ".docs"}

# Feature names that are correctly Title Case (exceptions to sentence-case rule).
# Used by check_header_case to avoid downcasing brand-name parts of headers.
PROPER_FEATURE_NAMES: set[str] = {
    "Slax", "Slax Reader", "Slax Note",
}

# Terminology: wrong → right (case-sensitive checks).
# Slax brand names must keep mixed case — see slax-home/CLAUDE.md "品牌与大小写规则".
PRODUCT_CASING = {
    "SLAX": ("Slax", "Brand uses mixed case 'Slax', not all caps"),
    "SLAX READER": ("Slax Reader", "Brand uses mixed case 'Slax Reader'"),
    "slax reader": ("Slax Reader", "Capitalize 'Slax Reader' as a product name"),
    "SLAX NOTE": ("Slax Note", "Brand uses mixed case 'Slax Note'"),
    "slax note": ("Slax Note", "Capitalize 'Slax Note' as a product name"),
}

# External product names (case-sensitive).
# Source: slax-home/CLAUDE.md "品牌与大小写规则".
EXTERNAL_CASING = {
    ...
    "iOs": ("iOS", "Lowercase i, capital OS"),
    "IOS": ("iOS", "Lowercase i, capital OS"),
    "iPados": ("iPadOS", "Use 'iPadOS'"),
    "Ipados": ("iPadOS", "Use 'iPadOS'"),
    "Tvos": ("tvOS", "Use 'tvOS'"),
    "TVOS": ("tvOS", "Use 'tvOS'"),
    "Tiktok": ("TikTok", "Use 'TikTok' (capital T's)"),
    "tikTok": ("TikTok", "Use 'TikTok'"),
    "TIKTOK": ("TikTok", "Brand uses mixed case 'TikTok'"),
    "Linkedin": ("LinkedIn", "Capital I in 'LinkedIn'"),
    "LINKEDIN": ("LinkedIn", "Brand uses mixed case 'LinkedIn'"),
    "A.I.": ("AI", "Use 'AI' not 'A.I.'"),
}

# Deprecated terminology (case-insensitive). Generic English-writing improvements.
DEPRECATED_TERMS = [
...
# Brand-specific terms to avoid. Empty for now; populate as Slax internal
# vocabulary settles (e.g., old feature names, anti-pattern phrases like
# "AI-powered" / "leverage" — see .agents/references/terminology.md
# "Anti-Patterns" section). Variable name kept for code-reference compatibility
# with the Warp original.
OZ_TERMS_TO_AVOID: list[tuple[str, str]] = []
...
def find_all_md_files() -> List[Path]:
    ...
    for f in [*DOCS_ROOT.rglob("*.md"), *DOCS_ROOT.rglob("*.mdx")]:
        if any(part in EXCLUDED_DIRS for part in f.parts):
            continue
        # Exclude changelog (historical record) — slax-home has two changelog
        # collections (reader-changelog, note-changelog).
        if any(f.is_relative_to(d) for d in CHANGELOG_DIRS):
            continue
        files.append(f)
    return sorted(files)
...
STRICT_DOCS_ROOT = DOCS_ROOT / "docs"


def _is_strict_docs(filepath: Path) -> bool:
    """True for files under src/content/docs/ (strict sentence-case headers +
    glossary-only bolded terms).

    False for marketing collections (blog, reader-alternatives,
    note-alternatives) where Title Case headers and ad-hoc emphasis bolding
    are intentional. slax-home currently has no strict docs path; this
    predicate is forward-compatible — when src/content/docs/ exists, files
    under it will automatically get the strict checks.
    """
    try:
        return filepath.is_relative_to(STRICT_DOCS_ROOT)
    except ValueError:
        return False
...
def run_all_checks(filepath: Path) -> List[Issue]:
    ...
    # Always-on checks (apply equally to marketing and product-docs content).
    issues = []
    ...
    # Strict-docs-only checks. Marketing content (blog / alternatives) uses
    # Title Case headers and ad-hoc bolding — neither is a defect.
    if _is_strict_docs(filepath):
